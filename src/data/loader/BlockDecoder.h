...
struct RegionData
{
    ...
    // 存储顺序（Litematica/Minecraft）：X最快变化，Z次之，Y最慢

    // 计算3D坐标对应的线性索引
    // 公式：x + z * sizeX + y * sizeX * sizeZ
    static constexpr int CalcBlockIndex(int x, int y, int z, int sizeX, int sizeY, int sizeZ)
    {
    ...
    {
        int idx = CalcBlockIndex(x, y, z, sizeX, sizeY, sizeZ);
    ...
    {
        uint16_t paletteIdx = GetBlockIndex(x, y, z);
